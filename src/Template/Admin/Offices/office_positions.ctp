                    ...
                    <div class="panel panel-inverse">
                        ...
                        <div class="panel-body">
                            <button type="button" class="row m-b-15 btn btn-yellow btn-sm" style="margin-left: 0.5%">
                                ...
                                <?= $this->Html->link('Add Office Position', ['action' => 'addOfficePosition']) ?>
                            </button>
                            <div id="data-table_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
                                <div class="row">
                                    ...
                                    <tr role="row">
                                        ...
                                        <tbody>
                                        <?php foreach ($office_positions as $i => $office_position): ?>
                                    <tr>
                                        <td>
                                            <?= $i + 1?>
                                        </td>
                                        <td>
                                            <?= $office_position->office_position_name ?>
                                        </td>
                                        <td>
                                            <?= $office_position->office_position_priority ?>
                                        </td>
                                        <td>
                                            <div class="pull-right center-block">
                                                <button type="button" class="btn btn-info btn-sm">
                                                    ...
                                                    <?= $this->Html->link('Edit', ['action' => 'editOfficePositions', $office_position->office_position_id]) ?>
                                                </button>
                                                <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete(<?php echo $office_position->office_position_id ?> )">
                                                    <i class="fa fa-trash">
                                                    </i>
                                                    Delete
                                                </button>
...
<!-- ================== END PAGE LEVEL JS ================== -->
    ...
    <script>
        ...
        function confirmDelete($office_position_id) {
            var targeturl = ' http://localhost' + '<?= \Cake\Routing\Router::url(["prefix" => "admin" ,"controller"=>"Offices","action"=>"deleteOfficePosition"]); ?>';
            var redirectURL = ' http://localhost' + '<?= \Cake\Routing\Router::url(["prefix" => "admin" ,"controller"=>"Offices","action"=>"officePositions"]); ?>';
            swal({
                title: "Are you sure?",
                text: "You want to delete office position?",
                type: "warning",
                ...
                cancelButtonText: "Cancel"
                    ...
                    if (isConfirm) {
                        $.ajax({
                            type:'post',
                            url: targeturl,              
                            data: {'office_position_id' : $office_position_id},
