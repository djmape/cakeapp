<!-- src/Template/Admin/Users/employees_all.ctp -->
        ...
        <div id="content" class="content">
            ...
            <ol class="breadcrumb pull-right">
                ...
                <li class="breadcrumb-item active">
                    <?php echo $this->Html->link('Administrators',['prefix' => "admin", 'controller' => 'Users','action'=>'employeeAll']) ?>
                </li>
            </ol>
            <!-- end breadcrumb -->
            <!-- begin page-header -->
            ...
            <div style="margin-bottom: 2%">
                <?= $this->Html->link('<i class="fa fa-plus"></i> Add Employee User', ['controller' => 'Users', 'action' => 'employeeAdd'],['class' => 'btn btn-yellow btn-sm','escape' => false]) ?>
            </div>
            <!-- begin table -->
            <table id="data-table-select" class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr class="odd gradeX" data-email-id="<?= $user->user_id ?>">
                        ...
                        <td>
                            <?php echo $this->Html->image("../webroot/img/upload/".$employee->user_employee_photo,['style' => 'max-width: 100%']); ?>
                        </td>
                        <td>
                            <?= $employee->user->email ?>
                        </td>
                        <td>
                            <?= $employee->user_employee_lastname . ', ' . $employee->user_employee_firstname . ' ' . substr($employee->user_employee_middlename,0 ,1) ?>
                        </td>
                        <td>
                            <div class="center-block">
                                <?= $this->Html->link('<i class="fa fa-edit"></i> Edit', ['controller' => 'Users', 'action' => 'employeeEdit', $employee->user_employee_id],['class' => 'btn btn-yellow btn-sm','escape' => false,'title' => $employee->user->email]) ?>
                                <button type="button" onclick="confirmDelete()" class="btn btn-danger btn-sm" data-user-id = "<?php echo $employee->user_id?>" id="btnDelete" title = "<?php echo $employee->user->email ?>">
...
<script>
    ...
    function confirmDelete() {
        ...
        var redirectURL = ' http://localhost' + '<?= \Cake\Routing\Router::url(["prefix" => "admin" ,"controller"=>"Users","action"=>"employeesAll"]); ?>';
        swal({
            title: "Are you sure?",
