        ...
        <div id="content" class="content">
            ...
            <?= $this->Html->link('<i class="fa fa-add"></i> Add Announcement', ['action' => 'announcementAdd'],['escape' => false, 'class' => 'btn btn-yellow btn-sm add-button' ]) ?>
            ...
            <table id="data-table-select" class="table table-bordered dataTable no-footer dtr-inline" role="grid" aria-describedby="data-table_info">
                <thead>
                    <tr role="row">
                        ...
                        <th class="sorting" tabindex="0" aria-controls="data-table" rowspan="1" colspan="1" aria-label="CSS grade: activate to sort column ascending" style="width: 9%;">
                            Actions
                        </th>
                    </tr>
                </thead>
                    ...
                    <?php foreach ($announcements as $announcement): ?>
                        <tr style="font-size: 12px">
                            ...
                            <td>
                                ...
                                ?>
                                        <?= preg_replace("/\<[^>]+\>/"," ",substr($announcement->announcement_body,0,100)) ?>
                                        . . .
                                <?php
                                    }
                                    else {
                                ?>
                                        <?= preg_replace("/\<[^>]+\>/"," ",$announcement->announcement_body) ?>
                                <?php
                                    }
                                    ?>
                            </td>
                            ...
                            <td>
                                <div class="pull-right center-block">
                                    <?= $this->Html->link('<i class="fa fa-edit"></i>', ['action' => 'edit', $announcement->announcement_id],['escape' => false, 'class' => 'btn btn-yellow btn-sm', 'title' => 'Edit ' . $announcement->announcement_title ]) ?>
                                    <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete(<?php echo $announcement->announcement_id ?> )" title="Delete <?php echo $announcement->announcement_title?>">
                                        <i class="fa fa-trash">
                                        </i>
                                    </button> 
                                </div> 
...
<!-- ================== END PAGE LEVEL JS ================== -->
    ...
    <script>
        ...
        function confirmDelete($announcement_id) {
            var targeturl = ' http://localhost' + '<?= \Cake\Routing\Router::url(["prefix" => "admin" ,"controller"=>"Announcements","action"=>"delete"]); ?>';
            var redirectURL = ' http://localhost' + '<?= \Cake\Routing\Router::url(["prefix" => "admin" ,"controller"=>"Announcements","action"=>"index"]); ?>';
            swal({
                title: "Are you sure?",
